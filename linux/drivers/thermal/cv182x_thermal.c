...
static int cv182x_read_temp(struct thermal_zone_device *tz, int *temperature)
{
	struct cv182x_thermal_zone *ctz = thermal_zone_device_priv(tz);
...
static const struct thermal_zone_device_ops cv182x_thermal_ops = {
...
static void cv182x_thermal_remove(struct platform_device *pdev)
{
...
static struct platform_driver cv182x_thermal_driver = {
	...
	.remove_new = cv182x_thermal_remove,
	.driver = {
		.name = "cv182x-thermal",
