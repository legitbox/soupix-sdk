...
static int cv180x_read_temp(struct thermal_zone_device *tz, int *temperature)
{
	struct cv180x_thermal_zone *ctz = thermal_zone_device_priv(tz);
...
static void cv180x_thermal_remove(struct platform_device *pdev)
{
...
static struct platform_driver cv180x_thermal_driver = {
	...
	.remove_new = cv180x_thermal_remove,
	.driver = {
		.name = "cv180x-thermal",
